...
usage() {
  echo "Usage: $0 --profile <aws-profile> --openai-key <key> --sender-email <email> --recipient-email <email> --openai-model <model>"
  exit 1
}
...
[[ -z "$AWS_PROFILE" || -z "$OPENAI_KEY" || -z "$SENDER_EMAIL" || -z "$RECIPIENT_EMAIL" || -z "$OPENAI_MODEL" ]] && usage
