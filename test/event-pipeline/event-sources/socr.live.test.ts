import { SocrSource } from '../../../src/event-pipeline/event-sources/socr.js';
...
jest.setTimeout(120_000); // 2 min — covers full detail fetches

describe_('SocrSource — live integration (LIVE=1 to run)', () => {
  let events: Event[] = [];

  beforeAll(async () => {
    const source = new SocrSource();
    events = await source.fetch();
  }, 120_000); // explicit timeout on beforeAll — jest.setTimeout alone does not cover it

  it('returns at least 1 event', () => {
    console.log(`\n[socr live] Total events returned: ${events.length}`);
  ...
  it('all events have required fields', () => {
    for (const event of events) {
      ...
      expect(event.url).toMatch(/^https:\/\/socr\.rozhlas\.cz\//);
      expect(event.sourceId).toBe('socr');
    }
  });

  it('at least one event has performers', () => {
    const withPerformers = events.filter(e => e.performers && e.performers.length > 0);
    console.log(`[socr live] Events with performers: ${withPerformers.length}/${events.length}`);
  ...
  it('at least one event has composers', () => {
    ...
    console.log(`[socr live] Events with composers: ${withComposers.length}/${events.length}`);
